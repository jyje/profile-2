...
블로그·위키는 같은 태그 사전을 사용합니다. 본문 아래의 문서 연결에서 들어오는 링크, 나가는 링크와 같은 태그의 글을 확인하고, 그래프에서 해당 문서의 주변 연결을 열 수 있습니다. 목록형 블로그 미리보기에는 이 영역을 반복하지 않습니다.

같은 컬렉션 안에서는 상대 경로 Markdown 링크를 사용합니다. 블로그와 위키 사이에는 `/wiki/knowledge/k8s`처럼 사이트 내 실제 주소를 사용합니다. 링크 텍스트나 제목이 같아도 주소가 다르면 별도 문서입니다. 코드 블록 안의 링크 예시는 관계로 계산하지 않습니다. 동적으로 계산한 JSX 링크는 그래프 대상이 아니므로 중요한 관계는 Markdown 링크로 작성하세요.

영문 번역이 없는 `.md`와 `.mdx` 위키 문서는 한국어 원문과 번역 권장 안내로 대체됩니다. 직접 작성한 영문 페이지는 유지됩니다. `draft: true`나 `unlisted: true` 문서는 공개 그래프와 공통 태그 목록에서 제외합니다.
